...
public sealed partial class DateTimeShould
{
	...
	public sealed class NotBeAfterTests
	{
		...
		public async Task WhenSubjectIsLater_ShouldFail()
		{
			...
			async Task Act()
				=> await That(subject).Should().NotBeAfter(expected);
			...
			await That(Act).Should().Throw<XunitException>()
				.WithMessage($"""
				              ...
				              but found {subject:O}
				              at Expect.That(subject).Should().NotBeAfter(expected)
				              """);
		}
		...
		public async Task Within_WhenValuesAreOutsideTheTolerance_ShouldFail()
		{
			...
			await That(Act).Should().Throw<XunitException>()
				.WithMessage($"""
				              ...
				              but found {subject:O}
				              at Expect.That(subject).Should().NotBeAfter(expected).Within(TimeSpan.FromSeconds(3))
				              """);
		}
