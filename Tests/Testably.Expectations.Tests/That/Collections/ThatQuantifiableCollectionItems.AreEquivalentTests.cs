...
public sealed partial class ThatQuantifiableCollectionItems
{
	public sealed class AreEquivalentTests
	{
		...
		public async Task WhenCollectionContainsOtherValues_ShouldFail()
		{
			...
			async Task Act()
				=> await ThatQuantifiableCollectionShould.BeEquivalentTo(Expect.That(subject).Should().All(), expected);
